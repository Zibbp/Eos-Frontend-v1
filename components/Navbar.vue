    ...
    <div class="container px-6 py-3 mx-auto md:flex">
      ...
      <div class="w-full md:flex md:items-center md:justify-between">
        ...
        <div class="reliative flex">
          ...
          />
          <button
            @click="$store.commit('toggleRandomVideos')"
            class="flex items-center px-2 py-2 font-medium tracking-wide text-neutral-700 capitalize transition-colors duration-200 transform hover:bg-slate-200 rounded-full"
          >
            <svg
...
export default {
  ...
  mounted() {
    ...
    const showRandomVideos = localStorage.getItem('showRandomVideos')
    if (showRandomVideos) {
      this.$store.commit('setShowRandomVideos', showRandomVideos)
    }
  },
  methods: {
    submitSearch() {
